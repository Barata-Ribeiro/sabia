...
export default function ReplyPostForm({ postId }: Readonly<{ postId: string }>) {
    ...
    const { isPending, formState, formAction, onSubmit } = useForm(postNewReply, {
        ok: false,
        clientError: null,
        response: null
    })
    ...
    useEffect(() => {
        if (formState.ok) {
            const postResponse = formState.response?.data as PostResponse
        ...
        <form action={formAction} onSubmit={onSubmit}>
            <div className="relative w-[32rem]">
                ...
                <div className="flex w-full justify-between py-1.5">
                    ...
                    >
                        {isPending ? t("ReplyPostButtonLoading") : t("ReplyPostButtonSubmit")}
                    </Button>
                </div>
            </div>
        </form>
